...
export async function computeDdRange(fromISO: string, toISO: string): Promise<MonthMetrics> {
  ...
  return {
    ...
    newOtherSales: R(ordNewSales - bNewSales - rNewSales - sgNewSales - newWa),
    totalNewSales: R(ordNewSales + newVipSales),
    repOtherSales: R(ordRepeatSales - bRepSales - rRepSales - sgRepSales - repWa),
    totalRepeatSales: R(ordRepeatSales + repVipSales),
