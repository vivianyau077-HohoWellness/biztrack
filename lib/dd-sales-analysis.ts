...
export type MonthMetrics = {
  ...
  newOtherSales: number; totalNewSales: number
  repOtherSales: number; totalRepeatSales: number
  ...
  sgNewSales: number; sgRepSales: number
}
...
export async function computeDdSalesMatrix() {
  ...
  const emptyRB = (): RB => ({ adB: 0, adR: 0, adSG: 0, adWA: 0, waAdB: 0, waAdR: 0, waAdSG: 0, ldB: 0, ldR: 0, ldSG: 0, pmB: 0, pmR: 0, pmSG: 0, wpmB: 0, wpmR: 0, wpmSG: 0, goal: 0, newOrder: 0, repeatOrder: 0, newSales: 0, repeatSales: 0 })
  ...
  for (const r of reportRecs) {
    ...
    y.waAdB += fnum(f['WA Ads Spend (焕肤王) (SST)']); y.waAdR += fnum(f['WA Ads Spend (钻石露) (SST)']); y.waAdSG += fnum(f['WA Ads Spend (SG) (SST)'])
    ...
    y.wpmB += fnum(f['WA PMed (焕肤王)']); y.wpmR += fnum(f['WA PMed (钻石露)']); y.wpmSG += fnum(f['WA Pmed (SG)'])
  ...
  const metrics: MonthMetrics[] = months.map(m => {
    ...
    return {
      ...
      newConv: Math.round(div(x.newOrder, totLd) * 1000) / 10,
      ...
      newOtherSales: R(x.newSales - x.bNewSales - x.rNewSales - x.sgNewSales - x.newWa),
      totalNewSales: R(x.newSales + x.newVipSales),
      repOtherSales: R(x.repeatSales - x.bRepSales - x.rRepSales - x.sgRepSales - x.repWa),
      totalRepeatSales: R(x.repeatSales + x.repVipSales),
      ...
      sgNewSales: R(x.sgNewSales), sgRepSales: R(x.sgRepSales),
    }
  })
